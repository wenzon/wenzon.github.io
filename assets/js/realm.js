...
function init(){
	var appId = 'oLBRRfcy7gCmaPGYlhf4vpWF-gzGzoHsz';
	var appKey = '7hOyoo9TQ7HFgYlw3UBGrAW7';
	var region = 'cn';

	AV.init({ appId: appId, appKey: appKey, region: region });
	
	let query_score = new AV.Query('GameScore');
	query_score.descending('score');
	query_score.limit(1);
	query_score.find().then(function (results) {
		var socre = results.length > 0 ? results[0].get('score') : 0;
		$("#highScore").text(socre);
		
		readyCount++;
		change_item();
	});
	
	let query_count = new AV.Query('GameScore');
	query_count.count().then(function (count) {
		$("#count").text(count);
	...
	let query_recent = new AV.Query('GameScore');
	query_recent.descending('createdAt');
	query_recent.limit(10);
	query_recent.find().then(function (results) {
		for(var i = 0; i < results.length; i++){
			var update = results[i].get('updatedAt').getTime();
			var score = results[i].get('score');
			datas.push({'x':update,'y':score});
		}
...
function change_item(){
	...
	isShow = true;		
	$("#loading").removeClass("container-show");
	$("#loading").addClass("container-hide");	
	$("#wrap").removeClass("container-hide");
	$("#wrap").addClass("container-show");
